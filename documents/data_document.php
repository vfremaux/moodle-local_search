 ...
 */
function data_make_link($databaseid, $recordid) {
    global $CFG;

    return new moodle_url('/mod/data/view.php', array('d' => $databaseid, 'rid' => $recordid));
}
 ...
 */
function data_get_records($databaseid, $typematch = '*', $recordid = 0) {
    global $CFG, $DB;
...
function data_get_content_for_index(&$database) {
    ...
    $params = array('course' => $database->course, 'module' => $coursemodule, 'instance' => $database->id);
 ...
 */
function data_check_text_access($path, $itemtype, $thisid, $user, $groupid, $contextid) {
    global $CFG, $DB;
