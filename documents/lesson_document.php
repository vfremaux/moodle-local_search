...
class LessonPageSearchDocument extends SearchDocument {
     ...
     *
     */
    public function __construct(&$page, $lessonmodule_id, $courseid, $itemtype, $contextid) {
...
function lesson_make_link($lessonmoduleid, $itemid, $itemtype) {
    ...
    if ($itemtype == 'page') {
        return new moodle_url('/mod/lesson/view.php', array('id' => $lessonmoduleid, 'pageid' => $itemid));
    }
    return new moodle_url('/mod/lesson/view.php', array('id' => $lessonmoduleid));
}
...
function lesson_get_content_for_index(&$lesson) {
    ...
    if ($pages) {
        ...
        $params = array('course' => $lesson->course, 'module' => $coursemodule, 'instance' => $lesson->id);
 ...
 */
function lesson_check_text_access($path, $itemtype, $thisid, $user, $groupid, $contextid) {
    global $CFG, $USER, $DB;
